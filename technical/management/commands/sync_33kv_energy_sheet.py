# technical/management/commands/sync_33kv_energy_sheet.py
"""
Management command: sync_33kv_energy_sheet

Syncs the active 33KV Energy Accounting Google Sheet for a given month into
CumulativeMeterReading (and via its save(), into EnergyDelivered).

Usage:
    python manage.py sync_33kv_energy_sheet --month 2026-07
    python manage.py sync_33kv_energy_sheet --month 2026-07 --force
    python manage.py sync_33kv_energy_sheet --month 2026-07 --day 15
    python manage.py sync_33kv_energy_sheet --month 2026-07 --dry-run
"""
...
class Command(BaseCommand):
    help = 'Sync 33KV Energy Accounting Google Sheet → CumulativeMeterReading + EnergyDelivered'
    ...
    def handle(self, *args, **options):
        ...
        if not feed:
            raise CommandError(
                f'No active 33KV Energy Accounting feed registered for {year}-{month:02d}. '
                f'POST to /api/tmo/sheet-feeds/ with feed_type=33kv_energy_accounting to register it.'
            )
        ...
        self.stdout.write(self.style.SUCCESS(f"CMR rows     : {result['cmr_rows']}"))
