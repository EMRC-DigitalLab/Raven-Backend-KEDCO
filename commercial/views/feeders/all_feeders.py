    ...
    return {
        ...
        'energy': {
            ...
            'actual_billed_kwh': metric(
                ...
                explanation='Energy billed from real meter readings only (estimation_method is empty) on this feeder.',
            ),
            'estimated_billed_kwh': metric(
                float(billing['estimated_billed_kwh'] + estimated['estimated_kwh']), unit='kWh', mode='estimated',
                explanation='Estimated energy: DataNest-estimated readings + Raven projection for customers with no reading on this feeder.',
            ),
            ...
            'energy_delivered_vs_billed': metric(
                ...
                explanation='Energy delivered vs billed for this feeder. Gap = delivered minus actual billed (raw, unscaled).',
            ),
        },
        'revenue': {
            ...
            'actual_total_billed': metric(float(billing['total_billed_amount']), unit='NGN', explanation='Revenue confirmed from customers on this feeder who were physically read this period. Calculated as billed consumption x tariff rate, plus 7.5% VAT. Sourced directly from DataNest meter readings.'),
            'estimated_revenue': metric(float(estimated['estimated_revenue']), unit='NGN', mode='estimated', explanation='Revenue estimated for customers on this feeder who were NOT read this period. For each unread customer, we take their last known billing amount, divide by the days that reading covered to get a daily rate, then multiply by the days in this period. This figure shrinks as more customers get read.'),
            'total_projected_revenue': metric(
                float(billing['total_billed_amount'] + estimated['estimated_revenue']), unit='NGN', mode='estimated',
                explanation='The full revenue picture for this feeder: actual billed (read customers) + estimated (unread customers). This is what should be collected if every customer on this feeder were read and billed this period. As reading coverage improves, the actual portion grows and the estimated portion shrinks.',
            ),
...
def all_feeders(request):
    ...
    energy_data    = bulk_energy_delivered(feeder_ids, date_range)
    ...
    for feeder in feeders:
        ...
        b_raw = billing_raw_data.get(fid, empty_billing())
        ...
        ed            = energy_data.get(fid, {'total_mwh': 0.0, 'mode': 'system'})
        ...
        consumed_kwh  = float(consumed_data.get(fid, ZERO))
        ...
        results.append({
            ...
            'energy': {
                ...
                'actual_billed_kwh':          metric(float(b['actual_billed_kwh']), unit='kWh', explanation='Energy billed from real meter readings only (estimation_method is empty) on this feeder.'),
                'estimated_billed_kwh':       metric(float(b['estimated_billed_kwh'] + e['estimated_kwh']), unit='kWh', mode='estimated', explanation='Estimated energy: DataNest-estimated readings + Raven projection for customers with no reading on this feeder.'),
                ...
                'energy_delivered_vs_billed': metric(
                    ...
                    unit='kWh', mode=ed['mode'], explanation='Energy delivered vs billed for this feeder. Gap = delivered minus actual billed (raw, unscaled).',
                ),
            },
            'revenue': {
                ...
                'actual_total_billed':     metric(float(b['total_billed_amount']), unit='NGN', explanation='Revenue confirmed from customers on this feeder who were physically read this period. Calculated as billed consumption x tariff rate, plus 7.5% VAT. Sourced directly from DataNest meter readings.'),
                'estimated_revenue':       metric(float(e['estimated_revenue']), unit='NGN', mode='estimated', explanation='Revenue estimated for customers on this feeder who were NOT read this period. For each unread customer, we take their last known billing amount, divide by the days that reading covered to get a daily rate, then multiply by the days in this period. This figure shrinks as more customers get read.'),
                'total_projected_revenue': metric(float(b['total_billed_amount'] + e['estimated_revenue']), unit='NGN', mode='estimated', explanation='The full revenue picture for this feeder: actual billed (read customers) + estimated (unread customers). This is what should be collected if every customer on this feeder were read and billed this period. As reading coverage improves, the actual portion grows and the estimated portion shrinks.'),
                'mdi_revenue_split':       metric(mdi_split,   unit='%', explanation='% of actual billed revenue from MDI customers on this feeder.'),
                'mdni_revenue_split':      metric(mdni_split,  unit='%', explanation='% of actual billed revenue from MDNI customers on this feeder.'),
                'arpu':                    metric(float(arpu), unit='NGN', explanation='Average Revenue Per Customer on this feeder.'),
            },
