...
def _band_metrics(band, customers_qs, readings_qs, date_range):
    ...
    return {
        ...
        'revenue': {
            ...
            'actual_total_billed':     metric(float(billing['total_billed_amount']), unit='NGN', explanation=f'Revenue confirmed from customers on Band {band.name} who were physically read this period. Calculated as billed consumption x tariff rate, plus 7.5% VAT. Sourced directly from DataNest meter readings.'),
            'estimated_revenue':       metric(float(estimated['estimated_revenue']), unit='NGN', mode='estimated', explanation=f'Revenue estimated for customers on Band {band.name} who were NOT read this period. For each unread customer, we take their last known billing amount, divide by the days that reading covered to get a daily rate, then multiply by the days in this period. This figure shrinks as more customers get read.'),
            'total_projected_revenue': metric(
                float(billing['total_billed_amount'] + estimated['estimated_revenue']), unit='NGN', mode='estimated',
                explanation=f'The full revenue picture for Band {band.name}: actual billed (read customers) + estimated (unread customers). This is what should be collected if every customer on Band {band.name} were read and billed this period. As reading coverage improves, the actual portion grows and the estimated portion shrinks.',
            ),
    ...
    for band in bands:
        ...
        results.append({
            ...
            'revenue': {
                ...
                'actual_total_billed':     metric(float(b['total_billed_amount']), unit='NGN', explanation=f'Revenue confirmed from customers on Band {band.name} who were physically read this period. Calculated as billed consumption x tariff rate, plus 7.5% VAT. Sourced directly from DataNest meter readings.'),
                'estimated_revenue':       metric(float(e['estimated_revenue']), unit='NGN', mode='estimated', explanation=f'Revenue estimated for customers on Band {band.name} who were NOT read this period. For each unread customer, we take their last known billing amount, divide by the days that reading covered to get a daily rate, then multiply by the days in this period. This figure shrinks as more customers get read.'),
                'total_projected_revenue': metric(float(b['total_billed_amount'] + e['estimated_revenue']), unit='NGN', mode='estimated', explanation=f'The full revenue picture for Band {band.name}: actual billed (read customers) + estimated (unread customers). This is what should be collected if every customer on Band {band.name} were read and billed this period. As reading coverage improves, the actual portion grows and the estimated portion shrinks.'),
                'mdi_revenue_split':       metric(mdi_split,   unit='%', explanation=f'% of actual billed revenue from MDI customers on Band {band.name}.'),
                'mdni_revenue_split':      metric(mdni_split,  unit='%', explanation=f'% of actual billed revenue from MDNI customers on Band {band.name}.'),
                'arpu':                    metric(float(arpu), unit='NGN', explanation=f'Average Revenue Per Customer on Band {band.name}.'),
            },
