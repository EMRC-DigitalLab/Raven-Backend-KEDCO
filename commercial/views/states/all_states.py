...
def _state_metrics(state, customers_qs, readings_qs, date_range):
    ...
    return {
        ...
        'revenue': {
            ...
            'actual_total_billed': metric(float(billing['total_billed_amount']), unit='NGN', explanation='Revenue confirmed from customers in this state who were physically read this period. Calculated as billed consumption x tariff rate, plus 7.5% VAT. Sourced directly from DataNest meter readings.'),
            'estimated_revenue': metric(float(estimated['estimated_revenue']), unit='NGN', mode='estimated', explanation='Revenue estimated for customers in this state who were NOT read this period. For each unread customer, we take their last known billing amount, divide by the days that reading covered to get a daily rate, then multiply by the days in this period. This figure shrinks as more customers get read.'),
            'total_projected_revenue': metric(
                float(billing['total_billed_amount'] + estimated['estimated_revenue']), unit='NGN', mode='estimated',
                explanation='The full revenue picture for this state: actual billed (read customers) + estimated (unread customers). This is what should be collected if every customer in this state were read and billed this period. As reading coverage improves, the actual portion grows and the estimated portion shrinks.',
            ),
    ...
    for state in states:
        ...
        results.append({
            ...
            'revenue': {
                ...
                'actual_total_billed':     metric(float(b['total_billed_amount']), unit='NGN', explanation='Revenue confirmed from customers in this state who were physically read this period. Calculated as billed consumption x tariff rate, plus 7.5% VAT. Sourced directly from DataNest meter readings.'),
                'estimated_revenue':       metric(float(e['estimated_revenue']), unit='NGN', mode='estimated', explanation='Revenue estimated for customers in this state who were NOT read this period. For each unread customer, we take their last known billing amount, divide by the days that reading covered to get a daily rate, then multiply by the days in this period. This figure shrinks as more customers get read.'),
                'total_projected_revenue': metric(float(b['total_billed_amount'] + e['estimated_revenue']), unit='NGN', mode='estimated', explanation='The full revenue picture for this state: actual billed (read customers) + estimated (unread customers). This is what should be collected if every customer in this state were read and billed this period. As reading coverage improves, the actual portion grows and the estimated portion shrinks.'),
                'mdi_revenue_split':       metric(mdi_split,       unit='%', explanation='% of actual billed revenue from MDI customers in this state.'),
                'mdni_revenue_split':      metric(mdni_split,      unit='%', explanation='% of actual billed revenue from MDNI customers in this state.'),
                'arpu':                    metric(float(arpu),     unit='NGN', explanation='Average Revenue Per Customer in this state.'),
            },
